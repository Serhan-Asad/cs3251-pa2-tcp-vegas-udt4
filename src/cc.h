...
class Vegas : public CCC {
  public:
    void init() {
        ...
        m_dCurrentRTT = m_dBaseRTT;
        m_dAlpha = 2.0;
        m_dBeta  = 4.0;
        m_dLinearIncreaseFactor = 0.125;
    ...
    void onACK(int32_t ack) {
        ...
        bool high_rtt_path=m_dBaseRTT>500.0; 
        if (!high_rtt_path) {
            if (m_dCWndSize<50.0) {
                m_dAlpha=2.0;
                m_dBeta=4.0;
                m_dLinearIncreaseFactor=0.125;
            } else if (m_dCWndSize<150.0) {
                m_dAlpha=3.0;
                m_dBeta=5.0;
                m_dLinearIncreaseFactor=0.25;
            } else {
                m_dAlpha=4.0;
                m_dBeta=6.0;
                m_dLinearIncreaseFactor=0.5;
            }
        } else {
            if (m_dCWndSize<50.0) {
                m_dAlpha=2.0;
                m_dBeta=4.0;
                m_dLinearIncreaseFactor=0.4;  
            } else if (m_dCWndSize<150.0) {
                m_dAlpha=3.0;
                m_dBeta=5.0;
                m_dLinearIncreaseFactor=0.6;  
            } else {
                m_dAlpha=4.0;
                m_dBeta =6.0;
                m_dLinearIncreaseFactor=0.8;  
            }
        }
        double cwnd_bytes = m_dCWndSize * m_iMSS;
        double expected_throughput =
            (cwnd_bytes * 1000.0) / m_dBaseRTT; 
        double actual_throughput =
            (cwnd_bytes * 1000.0) / m_dCurrentRTT; 

        double diff = ((expected_throughput - actual_throughput)
                       * m_dBaseRTT / 1000.0) / m_iMSS;
